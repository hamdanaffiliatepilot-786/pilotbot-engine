...
// ===== FIXED CORS - Only allow your domains =====
...
const strictLimiter = rateLimit({
  ...
  max: 20, // Stricter for AI tools
  message: { success: false, error: 'AI tool limit reached. Try again in an hour.' }
});

app.use('/api/tool/', strictLimiter);
...
console.log('Platform:', IS_VERCEL ? 'Vercel Serverless' : 'Traditional Server');
...
// FIXED: Less aggressive sanitization for text-heavy tools
function sanitizeStrict(input) {
    if (typeof input !== 'string') return input;
    // Only remove dangerous script-related content, keep quotes and basic HTML
...
function sanitizeForCode(input) {
    // For paragraph rewriter - preserve quotes and meaning
    if (typeof input !== 'string') return input;
...
async function sendTelegram(message, isChannel = false) {
    ...
    try { 
        ...
        console.error('Telegram error:', e.message?.substring(0, 50));
    }
}

// ===== DUAL AI ENGINE (Groq + Gemini) =====
// FIXED: Reduced timeout for Vercel compatibility
...
async function askAI(prompt, retries = 2) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            console.log(`Groq attempt ${attempt + 1} failed:`, e.message?.substring(0, 100));
...
async function pingIndexNow(url) {
    try {
        await axios.post('https://api.indexnow.org/IndexNow', 
            { host: new URL(url).hostname, key: 'pilotbotindexkey123', urlList: [url] }, 
            { timeout: 5000 }
        );
    } catch (e) {}
}
...
function err(res, msg, code) { 
    console.error(`Error ${code || 500}:`, msg);
...
app.get('/api/health', (req, res) => ok(res, {
    success: true,
    platform: IS_VERCEL ? 'Vercel Serverless' : 'Traditional',
...
app.get('/api/public-stats', async (req, res) => {
    ...
    try {
        ...
        ok(res, { success: true, activeUsers: '2.1K+', totalTasks: '15K+' });
    }
});
...
const toolRoutes = [
    ...
    { path: 'paragraph-rewriter', prompt: (t) => `Rewrite this professionally keeping exact meaning: "${t}". Better vocabulary, improved flow, same facts. OUTPUT ONLY TEXT.`, sanitize: 'code' },
...
toolRoutes.forEach(route => {
    app.post(`/api/tool/${route.path}`, async (req, res) => {
        // FIXED: Use appropriate sanitizer based on tool type
        let input;
        if (route.sanitize === 'code') {
            input = sanitizeForCode(req.body.topic || req.body.prompt);
        } else {
            input = sanitizeStrict(req.body.topic || req.body.prompt);
        }
        ...
        console.log(`Tool ${route.path} called with: ${input.substring(0, 50)}...`);
...
app.post('/api/agent/content-writer', async (req, res) => {
    ...
    for (let i = 0; i < Math.min(count, 5); i++) {
        ...
        if (html) {
            ...
            results.push({ title: tm ? tm[1].replace(/<[^>]*>/g, '') : topic, content: html, words: html.split(/\s+/).length });
        }
    }
...
app.post('/api/ai-chat', async (req, res) => {
    ...
    const reply = await askAI(`You are a helpful AI assistant for PilotStaff, a platform with 30+ free AI tools and AI employees. User says: "${message}". Be concise, helpful, and mention relevant tools when appropriate. Respond in HTML format.`);
    if (!reply) return err(res, 'AI failed', 503);
    ok(res, { success: true, reply });
});

// CRM Leads endpoint for Dashboard
app.get('/api/crm/leads', async (req, res) => {
    if (!supabase) {
        return ok(res, { success: true, leads: [] });
    }
    try {
        ...
        ok(res, { success: true, leads: leads || [] });
    } catch (e) {
        ok(res, { success: true, leads: [] });
    }
});

// PayPal Webhook endpoint
app.post('/api/paypal-webhook', async (req, res) => {
    const { orderID, payerID, plan, price, payerEmail } = req.body;
    
    console.log('PayPal webhook received:', { orderID, plan, price, payerEmail });
...
1. Early: <a href="${WEBSITE_URL}" style="color:#2563eb;font-weight:600;text-decoration:underline;">PilotStaff</a>
2. Middle: <a href="${WEBSITE_URL}/tools" style="color:#2563eb;font-weight:600;text-decoration:underline;">25 free AI tools</a>
...
- End with: "Check out <a href="${WEBSITE_URL}" style="color:#2563eb;font-weight:600;text-decoration:underline;">PilotStaff.com</a> to explore their free AI tools."

RULES:
- Every link must feel NATURAL in the sentence
...
app.post('/api/trigger-blog', async (req, res) => {
    if (!BLOG_ID || !BLOGGER_REFRESH_TOKEN) {
        return err(res, 'Blogger not configured. Set BLOGGER_REFRESH_TOKEN and BLOG_ID in env.', 400);
    }
    const topic = TRENDING_TOPICS[Math.floor(Math.random() * TRENDING_TOPICS.length)];
    try {
        const title = await publishSEOBlog(topic);
    ...
    try {
        ...
        if (!token) {
            return ok(res, {
                ...
                error: 'Auth failed',
                debug: {
                    token: !!BLOGGER_REFRESH_TOKEN,
                    clientId: !!BLOGGER_CLIENT_ID,
                    secret: !!BLOGGER_CLIENT_SECRET,
                    blogId: BLOG_ID || 'NOT SET'
                }
            });
        }
        const { data } = await axios.get(`https://www.googleapis.com/blogger/v3/blogs/${BLOG_ID}/posts?maxResults=1`, {
            headers: { Authorization: `Bearer ${token}` },
            timeout: 10000
        });
        ok(res, {
            ...
            lastPost: data.items?.[0]?.title || 'None',
            blogUrl: `https://${BLOG_ID}.blogspot.com`
        });
    } catch (e) {
        ok(res, {
            connected: false,
...
app.get('/api/get-old-posts', async (req, res) => {
    ...
    try {
        const token = await getBloggerToken();
        if (!token) return err(res, 'Auth failed', 401);
        const { data } = await axios.get(`https://www.googleapis.com/blogger/v3/blogs/${BLOG_ID}/posts?maxResults=10`, {
            headers: { Authorization: `Bearer ${token}` },
            timeout: 15000
        });
        ok(res, {
...
// ===== CRON: Only for non-Vercel deployments =====
if (!IS_VERCEL) {
    ...
    cron.schedule('0 4,16 * * *', async () => {
        ...
        lastAutoBlog = topic;
        try {
            await publishSEOBlog(topic);
            console.log('✅ Cron blog:', topic.substring(0, 40));
        } catch (e) {
            console.error('❌ Cron blog failed:', e.message);
        }
    });

    cron.schedule('*/30 * * * *', () => {
        console.log(`💓 ${new Date().toLocaleTimeString()} | ${Math.round(process.memoryUsage().heapUsed/1024/1024)}MB`);
    });
    
    console.log('✅ Cron jobs scheduled (traditional server mode)');
} else {
    console.log('⏭️ Skipping cron jobs (Vercel serverless mode)');
}
...
// ===== EXPORT FOR VERCEL / LISTEN FOR TRADITIONAL =====
const PORT = process.env.PORT || 3000;

if (IS_VERCEL) {
    module.exports = app;
    console.log('🚀 Exporting app for Vercel Serverless Functions');
} else {
    app.listen(PORT, () => {
        console.log(`🤖 PilotStaff API on http://localhost:${PORT}`);
