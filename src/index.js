...
app.post(
    ...
    async (req, res) => {
        try {
            ...
            logger.error("Automation error:", e.message || e);
...
app.use((req, res) => {
    logger.warn(
        `[${req.requestId || "-"}] Endpoint not found: ${req.method} ${req.originalUrl}`
    );
...
app.use((error, req, res, next) => {
    ...
    logger.error(
        `[${rid}][ERR:${eid}] ${req.method} ${req.originalUrl}: ${error.message}`
    );
